*** Settings ***
Resource        ../../../../resource/resource_login.robot
Resource        ../../../../resource/app/resource_frmProcessoAutomatico.robot
Resource        ../../../../resource/app/resource_cnsProcessoAutomatico.robot

Test Setup      Acessar Suricato
Test Teardown   Fechar Navegador
#Suite Teardown  Enviar Resultado  ${SUITE SOURCE}  ${OBJETIVO}  ${SUITE STATUS}

*** Variables ***
${OBJETIVO}    Verificar todas os campos na Tela: Processo On-Line (Filtro)
${SCREENNAME}  Processo On-Line
${APPNAME}     cnsProcessoAutomatico

*** Test Cases ***
Testcase: Verificar todas os campos na Tela: Processo On-Line (Filtro)
# Online #
  [Tags]   VERIFY  246  247 
  [Documentation]   ${OBJETIVO}
  
  Dado que eu acesse o menu: Processo | Online
  Quando eu visualizar a tela: Processo On-Line (Consulta)
  E clicar no botão: Pesquisar Registros
  Então devo visualizar a tela: Processo On-Line (Filtro)
  E verificar a label: Pesquisar
  E verificar o campo: Código
  E verificar o campo: Descrição
  E verificar se existe o campo Teste na tela
 

*** Keywords ***
Dado que eu acesse o menu: Processo | Online
  resource_mnu.Clicar No Menu Processo | Online

Quando eu visualizar a tela: Processo On-Line (Consulta)
  resource_cnsProcessoAutomatico.Acessar Tela Processo On-Line

E clicar no botão: Pesquisar Registros
    resource_btn.Clicar No Botão Pesquisar Registros

Então devo visualizar a tela: Processo On-Line (Filtro)
    resource_cnsProcessoAutomatico.Acessar Tela Processo On-Line (Filtro)

E verificar a label: Pesquisar
    resource_cnsProcessoAutomatico.Verificar a label: Pesquisa

E verificar o campo: Código
    resource_cnsProcessoAutomatico.Verificar o campo: Código

E verificar o campo: Descrição  
    resource_cnsProcessoAutomatico.Verificar o campo: Descrição

E verificar se existe o campo Teste na tela  
    resource_utils.Verificar Campo Teste na Tela "${SCREENNAME}" da Aplicação "${APPNAME}"

